...
impl XWin
{
	...
	///
	/// This function will not return until XWin is terminated, which happens
	/// automatically after the closure passed to this function terminates.
	///
	/// XWin may also be terminated early by calling [XWin::terminate], which
	/// will immediately close all windows and cause this function to return.
	///
	/// # Returns
	...
	pub fn terminate()
	{
		...
		{
			let _ = xwin.tx.send(XWinMessage::Terminate);
		}
	}
