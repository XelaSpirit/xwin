...
/// Contains the configuration for a [Window]. May also be used to update the
/// window's configuration.
///
/// Two `WindowConfig`'s are said to be equal if and only if they represent the
/// same window.
pub struct WindowConfig<'a>(&'a mut Window);

impl PartialEq for WindowConfig<'_>
{
...
impl<'a> WindowConfig<'a>
{
	...
	pub fn try_title(&self) -> Result<String, XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::GetWindowTitle(self.0.as_glfw(), tx), rx)?
	}
	...
	pub fn try_set_title(&mut self, title: &str) -> Result<(), XErr>
	{
		...
		XWin::get()?.read().unwrap().post_rcv(
			XWinMessage::SetWindowTitle(self.0.as_glfw(), String::from(title), tx),
			rx,
		)?
	}
	...
	pub fn try_set_icon(&mut self, icons: Vec<Image>) -> Result<(), XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::SetWindowIcon(self.0.as_glfw(), icons, tx), rx)?
	}
	...
	pub fn try_framebuffer_size(&self) -> Result<Pixels, XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::GetFrameBufferSize(self.0.as_glfw(), tx), rx)?
	}
	...
	pub fn try_frame_size(&self) -> Result<(u32, u32, u32, u32), XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::GetWindowFrameSize(self.0.as_glfw(), tx), rx)?
	}
	...
	pub fn try_content_scale(&self) -> Result<ContentScale, XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::GetWindowContentScale(self.0.as_glfw(), tx), rx)?
	}
	...
	pub fn try_opacity(&self) -> Result<f32, XErr>
	{
		...
		XWin::get()?
			.read()
			.unwrap()
			.post_rcv(XWinMessage::GetWindowOpacity(self.0.as_glfw(), tx), rx)?
	}
	...
	pub fn try_set_opacity(&mut self, opacity: f32) -> Result<(), XErr>
	{
		let (tx, rx) = channel();
		XWin::get()?.read().unwrap().post_rcv(
			XWinMessage::SetWindowOpacity(self.0.as_glfw(), opacity, tx),
			rx,
		)?
	}
	...
	{
		self.0.attr(GLFW_TRANSPARENT_FRAMEBUFFER)
	}
	...
	{
		self.0.attr(GLFW_FOCUS_ON_SHOW)
	}
	...
	{
		self.0.set_attr(GLFW_DECORATED, value)
	}
	...
	{
		self.0.set_attr(GLFW_RESIZABLE, value)
	}
	...
	{
		self.0.set_attr(GLFW_FLOATING, value)
	}
	...
	{
		self.0.set_attr(GLFW_AUTO_ICONIFY, value)
	}
	...
	{
		self.0.set_attr(GLFW_FOCUS_ON_SHOW, value)
	}
